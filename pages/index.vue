<template>
	<article id="index">
        <IndexHead v-if="saying.length" :saying="saying"></IndexHead>
				...
				<div class="white-card article-class">
					...
					<ul>
						<li v-for="(item, index) in webside.tags_count" :key="index">
							<nuxt-link tag="span" :to="{path: '/categories', query: { class: index }}">{{ index | articleTag }}</nuxt-link>
							<span>{{ item }}</span>
						</li>
					</ul>
				</div>
...
export default {
	...
	head () {
		return {
			title: '夏叶的技术博客-主页',
			meta: [
				{ hid: 'description-index', name: 'description', content: '夏叶，夏叶博客，夏叶的技术博客, 主页' }
			]
		}
	},
	data () {
	...
	mounted () {
		...
		for ( let prop in JSON.parse(this.webside.sentences) ) {
			this.saying.push(JSON.parse(this.webside.sentences)[prop])
		}
	},
	computed: {
		...mapState({
